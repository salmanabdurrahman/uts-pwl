...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        try {
            ...
            if (empty($title) || empty($short_description) || empty($content) || empty($article_id)) {
                swal_error(
                    'Error',
            ...
            if ($image_url) {
                $stmt = $conn->prepare("UPDATE articles SET title = ?, short_description = ?, content = ?, image_url = ? WHERE article_id = ?");
                if (!$stmt) {
                    throw new Exception("Database preparation failed");
                }
                $stmt->bind_param("ssssi", $title, $short_description, $content, $image_url, $article_id);
            } else {
                $stmt = $conn->prepare("UPDATE articles SET title = ?, short_description = ?, content = ? WHERE article_id = ?");
                if (!$stmt) {
                    throw new Exception("Database preparation failed");
                }
                $stmt->bind_param("sssi", $title, $short_description, $content, $article_id);
            }
            ...
            if ($stmt->affected_rows === 1) {
                ...
                throw new Exception("Content update failed. Please try again.");
            }
        } catch (Exception $e) {
